...
class ProductionTester3B7 {
  ...
  async step1_CheckCompartment3B7() {
    ...
    if (!this.compartment3B7) {
      ...
      await this.log("📋 Available compartments:", compartments.map(c => c.address));
  ...
  async step3_TestEntryMovement() {
    await this.log("STEP 3: Testing ENTRY movement - Adding 100 units to 3B7");
    
    const movementData = {
      user_id: "11111111-1111-1111-1111-111111111111", // Test UUID
      product_id: this.testProduct.id,
      compartment_address: this.compartment3B7.address, // Use address instead of id
      tipo: "ENTRADA",
      qty: 100,
      obs: "Teste automatizado - Entrada 3B7"
    };

    const response = await this.apiRequest('/api/movements/create', {
      method: 'POST',
  ...
  async step4_TestExitMovement() {
    await this.log("STEP 4: Testing EXIT movement - Removing 30 units from 3B7");
    ...
    const movementData = {
      ...
      tipo: "SAIDA",
      qty: 30,
      obs: "Teste automatizado - Saída 3B7"
    };
  ...
  async generateFinalReport() {
    console.log('\n' + '='.repeat(80));
    console.log('📋 RELATÓRIO FINAL - TESTE AUTOMATIZADO COMPARTIMENTO 3B7');
  ...
  async runCompleteTest() {
    try {
      console.log('🚀 INICIANDO TESTE AUTOMÁTICO COMPLETO - COMPARTIMENTO 3B7\n');
...
// Execute the test
const tester = new ProductionTester3B7();
